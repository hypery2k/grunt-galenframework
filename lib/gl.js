...
gl = function () {
   ...
   * @param {Object} device device specification
   * @param {String} code   javascript code that need to be injected
   */
  function injectCode(device, code) {
    var slGrid = config.getSeleniumGrid();
    var singleDevice = config.getSingleDeviceState();
    var targetDevice;
    if (slGrid.enabled === false) {
   ...
   * @param {Object}      settings              (optional) a set of key/value that specifies different properties for checking layout
   * @param {String[]}    settings.excludedTags (optional) an array of tags that should be excluded when checking layout
   * @param {String}      settings.screenshot   (optional) a path to a screenshot. In case this property is provided Galen will not make a screenshot and will use the provided screenshot instead in reports and for image analysis
   * @param {Object}      settings.vars         (optional) a set of key value pairs which will be used in spec as variables
   * @param {Object}      settings.objects      (optional) a set of object locators which can be referenced from spec
   */
  function runSpecFile(device, specFile, tags, settings) {
    var useSingleDevice = config.getSingleDeviceState() && !config.getSeleniumGrid().enabled;

    settings = settings || {};

    checkLayout({
      driver: _drivers[useSingleDevice ? 0 : device.deviceName],
      spec: specFile,
      tags: tags,
